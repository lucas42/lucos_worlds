...
class OidcProviderSettings
{
     ...
     *
     * lucos patch (lucas42/lucos_worlds#26, decision in #21): additionally accept
     * EC/ES256 keys (specifically P-256, alg ES256), since our OIDC provider
     * (lucos_aithne) signs exclusively with ES256 and publishes no RSA key.
     * Upstream BookStack is RSA/RS256-only by design (see BookStackApp/BookStack#5390,
     * open/unresolved as of 2026-07). This filter must stay in sync with the
     * companion patches in OidcJwtWithClaims::validateTokenSignature() and
     * OidcJwtSigningKey — all three gate the same RSA-only restriction
     * independently, and all three must accept EC/ES256 for login to work.
     */
    protected function filterKeys(array $keys): array
    {
        return array_filter($keys, function (array $key) {
            ...
            if ($key['kty'] === 'RSA' && $alg === 'RS256') {
                return true;
            }

            if ($key['kty'] === 'EC' && $alg === 'ES256' && ($key['crv'] ?? null) === 'P-256') {
                return true;
            }
