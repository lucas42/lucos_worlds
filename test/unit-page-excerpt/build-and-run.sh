...
# Fast, no-network unit test of Page::getExcerpt()'s first-line-aware
# truncation (lucas42/lucos_worlds#52) — no docker-compose stack needed.
set -eu
cd "$(dirname "$0")"
...
docker run --rm --entrypoint /app/www/vendor/bin/phpunit lucos_worlds_unit_test \
    --bootstrap /app/www/vendor/autoload.php --no-configuration /tmp/PageExcerptTest.php
